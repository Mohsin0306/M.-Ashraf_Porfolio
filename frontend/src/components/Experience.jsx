      ...
      <div className="max-w-3xl mx-auto">
        <FadeIn>
          <div className="flex items-center gap-3 mb-8 md:mb-10">
            <span className="px-3 py-1 rounded-full bg-[#FA8112]/20 text-[#FA8112] text-sm font-bold">02</span>
            <h2 className="text-2xl md:text-4xl font-bold text-[#222222]">Experience</h2>
          </div>

          <div className="space-y-6 md:space-y-8">
            {/* Main role */}
            <div className="p-5 md:p-6 rounded-2xl bg-white/80 border border-[#FA8112]/15 shadow-sm">
              <h3 className="text-lg md:text-xl font-bold text-[#222222]">{experience.main.title}</h3>
              <p className="text-[#FA8112] font-semibold text-sm md:text-base mt-1">{experience.main.role}</p>
              ...
              <ul className="mt-4 space-y-3">
                {experience.details.items.map((item, i) => (
                  <li key={i} className="flex gap-3 text-sm md:text-base">
                    <span className="w-1.5 h-1.5 rounded-full bg-[#FA8112] shrink-0 mt-2" />
                    <span>
